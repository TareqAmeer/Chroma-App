...
(function () {
  ...
  // Ultra HDR JPEG export shipped on Windows too — docs/windows-port.md Phase 4). Anything
  // already rendered off the sniffed guess re-renders on the 'cs-platform-ready' event fired
  // once the correction lands.
  const macGuess = /Mac/.test(navigator.platform || '');
  window.CS_PLATFORM = {
    os: macGuess ? 'macos' : 'windows',
    hdrExport: macGuess ? 'heic' : 'uhdr',
    eject: true,
    haptics: macGuess,
    fastThumb: true,
    videoPoster: true,
