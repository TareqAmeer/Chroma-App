...
both silent or misleading — see CLAUDE.md §3. This skill exists so neither has to be
...
those `vidSample`/`vidOut` per CLAUDE.md §12.)
...
this is a known-flaky WebGL context loss on this machine (CLAUDE.md §"Two tests on this
...
CLAUDE.md §12's video-work rule generalized to any shader work — all `test/golden/` PNGs stay
