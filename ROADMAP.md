...
| R3 | Spektrafilm profiles as presets — tier (a) | S | Bake 35 stocks × 11 papers to `.cube` into `vendor/luts/`+`LUT_META`. Purely additive, zero engine change. Bakes to display space (colour, not roll-off). CC BY-SA 4.0 → credit in `LICENSES-MODELS.md`. |
