...
const md = [];
md.push('# Swiss Kinetic redesign — UI inventory', '', `Generated by \`node scripts/redesign-inventory.mjs\` from a live walk of ${results.length} app states. Do not hand-edit the lists; re-run the scan. Mark each row's **Decision** as \`new\` (new design), \`restyle\` (tokens only) or \`leave\`.`, '');
md.push('## 1. Component families (registry)', '', '| Family | Max visible in one state | Decision |', '|---|---|---|');
for (const [f] of FAMILIES) md.push(`| ${f} | ${famMax[f] || 0} | |`);
md.push(`| icon (glyphs) | ${registry.families?.icon ?? '?'} in source | |`);
...
for (const s of special) md.push(`| \`${s.sig}\` | ${s.states} | ${s.cursor} | ${s.hosts.join(', ')} | ${s.examples.join(' / ').replace(/\|/g, '/')} | |`);
...
for (const s of surfaces) md.push(`| ${s.id} | ${s.reachedInWalk ? 'yes' : 'no'} | |`);
...
for (const [name, re] of [['Scrollbars', /::-webkit-scrollbar/g], ['Focus ring (:focus-visible)', /:focus-visible/g], ['Tooltips (title=)', /\btitle="/g], ['Text selection (::selection)', /::selection/g], ['Progress bars / spinners', /progress|spin\b|@keyframes \w*spin/gi], ['Toasts', /fx-toast|lib-toast/g], ['Star / flag / reject markers', /lib-flag|lib-star|rating/g], ['Histogram', /fx-hist/g]]) md.push(`| ${name} | ${cnt(re)} | |`);
md.push('| Window buttons (native, to become custom squares) | Tauri config | new |', '| Wordmark CHRO-MA-SMITH | new | new |', '| Splash / loading screen | 1 surface | |');
