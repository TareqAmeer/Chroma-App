...
const res=await pg.evaluate(async()=>{
  ...
  const btn=card.querySelector('.fx-ctrl-title-reset');
  const btnExists=!!btn;
  const btnVisible=btn?getComputedStyle(btn).display!=='none':null;
  if(btn)btn.click();
  await wait();
  const afterClick=sl.value;
  return {afterEdit, btnExists, btnVisible, afterClick};
});
