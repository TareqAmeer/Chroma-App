...
calib/                  Calibration tooling (Python) — calib/AGENTS.md. Not needed to run the app.
...
- **Build stamp:** `chromasmith-22.html` has `const BUILD='YYYY-MM-DDx'` near the top of its
  ...
  A `PostToolUse` hook (`.codex/hooks.json`) auto-bumps it to today's date on any Edit/Write
...
### Calibration tooling
Setup + commands in [calib/AGENTS.md](calib/AGENTS.md) (auto-loads inside `calib/`); skill `chromasmith-calib`.
...
- **[calib/AGENTS.md](calib/AGENTS.md)** — halation/bloom emission model science, `calib/`
...
See [calib/AGENTS.md](calib/AGENTS.md) for chart zone pixel coordinates and the
