...
something — see AGENTS.md §6 lesson #1/#10 for why this is the cheaper and more reliable
signal.
...
AGENTS.md's engine-mismatch note. Treat a clean run as "no Chromium-visible regression," not as
...
- A shader edit that might not compile at all → `node test/export_harness.mjs` first (catches
  GLSL compile errors and the "quieter" no-op class from AGENTS.md §3), then the scorecard for
  correctness.
- A change to Library/native-gated UI → the `?libtest=1` browser harness (AGENTS.md §6 item 14),
...
- Anything where the numbers pass but you're not sure it *looks* right → still do one
  render-and-look pass per AGENTS.md §6 lesson #1; the scorecard is a fast secondary
