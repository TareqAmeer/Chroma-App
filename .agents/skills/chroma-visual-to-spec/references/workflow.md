...
Use deterministic pixel/color evidence before model estimates. First create a named crop manifest in original-image coordinates and retain its measurement JSON beside the reference evidence. `npm run reference:measure -- <png> <output-dir> [x,y ...] [--crop id:x,y,width,height ...]` records the PNG SHA-256, dimensions, exact RGBA samples, and per-crop mean/min/max pixel statistics. These are pixel measurements, not intended design tokens. Treat shadows, blur, alpha, and antialiasing as ranges unless exact deterministic evidence supports a value. WebP and semantic interpretation remain unsupported. Keep fixed screenshot geometry separate from inferred responsive rules.

When a later approved implementation has same-size PNG evidence, run `npm run reference:compare -- <baseline.png> <candidate.png> <output-dir>`. Record the pixelmatch count/ratio and raw RGBA changed bounds as validation evidence; a delta identifies pixels, not correctness. Dimension mismatches, missing files, and unseen states are unresolved evidence gaps, not conclusions.
